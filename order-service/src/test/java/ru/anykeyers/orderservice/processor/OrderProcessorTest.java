...
/**
 * Тесты для класса {@link OrderProcessor}
 */
...
public class OrderProcessorTest {
    ...
    @BeforeEach
    public void setUp() {
        orderProcessor = new OrderProcessor(orderRepository, Executors.newVirtualThreadPerTaskExecutor());
    }
    ...
    void verifyProcessingOrders() {
        ...
        Assertions.assertEquals(OrderState.PROCESSING, processingOrder.getStatus());
        Mockito.verify(orderRepository, Mockito.never()).save(waitingOrder);
        Assertions.assertEquals(OrderState.WAIT_PROCESS, waitingOrder.getStatus());
    }
    ...
    void verifyProcessedOrders() {
        ...
        Assertions.assertEquals(OrderState.PROCESSED, processedOrder.getStatus());
    }
